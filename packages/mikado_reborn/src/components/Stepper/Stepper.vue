  ...
  <div>
    <div class="mkr__stepper">
      ...
      <div class="mkr__stepper-header">
        <template
          v-for="({ type, label }, i) in itemsAsObject"
          :key="i"
        >
          <!-- Header Divider -->
          <div
            v-if="i !== 0"
            class="mkr__stepper-header__divider"
          />
          <!-- Header Item -->
          <div
          ...
          >
            ...
            <div
              v-else
              class="mkr__stepper-header__item-number"
            >
              {{ i + 1 }}
            </div>
...
export type StepperItem = { type: "default" | "error" | "warning"; label: string };
...
const itemsAsObject = computed(() => props.items?.map((item: string | StepperItem) => {
